...
# 2. ESTADOS DE SESIÓN (Persistencia de alertas y tiempo)
...
# --- PANEL LATERAL (DIAGNÓSTICO Y ALARMAS) ---
with st.sidebar:
    st.header("💰 Cuenta")
    ...
    # 👇 ESTE ES EL CONTEO DE 1 HORA (REINTEGRADO Y VISIBLE)
...
# --- LÓGICA DE POSICIÓN ACTIVA Y CIERRES ---
...
# --- HEARTBEAT 1 HORA ---
ahora = datetime.now()
if (ahora - st.session_state.ultima_alerta_vida) > timedelta(hours=1):
...
if c1.button("🟢 MANUAL LONG"):
    ...
    client.registrar_trade("LONG", precio_actual, 0, 0)
    st.rerun()
if c2.button("🔴 MANUAL SHORT"):
    client.place_order("BTCUSDT", "SELL", str(cant_m))
    client.registrar_trade("SHORT", precio_actual, 0, 0)
    st.rerun()
