...
# 2. ESTADOS DE SESIÓN (Incluye memoria de alertas)
...
if 'alerta_long_enviada' not in st.session_state: st.session_state.alerta_long_enviada = False
if 'alerta_short_enviada' not in st.session_state: st.session_state.alerta_short_enviada = False

st.title(f"🤖 Terminal Pro - BTC: ${precio_actual:,.2f}")

# --- PANEL LATERAL (DIAGNÓSTICO Y ALERTAS) ---
with st.sidebar:
    st.header("💰 Cuenta")
    ...
    # --- LÓGICA DE DIAGNÓSTICO Y ALERTAS TELEGRAM ---
    if 35 < rsi < 55:
        st.write("🟡 **RSI Neutral:** Esperando caída para **LONG (35)** o subida para **SHORT (55)**.")
        st.session_state.alerta_long_enviada = False
        st.session_state.alerta_short_enviada = False

    elif rsi <= 35:
        st.session_state.alerta_short_enviada = False # Reset short
        if precio_actual > ema:
            msg = "🎯 **Oportunidad LONG:** RSI en zona de compra y sobre la EMA."
            st.success(msg)
            if not st.session_state.alerta_long_enviada:
                client.enviar_telegram(msg)
                st.session_state.alerta_long_enviada = True
        else:
            st.warning("🔴 **Filtro EMA:** RSI listo para **LONG (35)**, pero el precio está bajo la EMA.")

    elif rsi >= 55:
        st.session_state.alerta_long_enviada = False # Reset long
        if precio_actual < ema:
            msg = "🎯 **Oportunidad SHORT:** RSI en zona de venta y bajo la EMA."
            st.success(msg)
            if not st.session_state.alerta_short_enviada:
                client.enviar_telegram(msg)
                st.session_state.alerta_short_enviada = True
        else:
