...
st.title("🤖 Terminal Trading - MODO FULL AUTO + ALERTAS")
...
with st.sidebar:
    ...
    # RESTAURADOS: Take Profit y Stop Loss fijos
    ...
    # RESTAURADOS: Indicadores en la barra lateral
...
if posicion:
    ...
    if precio_actual > 0:
        ...
        # 1. CIERRE POR LÍMITES FIJOS (TP/SL)
        if (side == "LONG" and ((tp_input > 0 and precio_actual >= tp_input) or (sl_input > 0 and precio_actual <= sl_input))) or \
           (side == "SHORT" and ((tp_input > 0 and precio_actual <= tp_input) or (sl_input > 0 and precio_actual >= sl_input))):
            ...
            client.enviar_telegram(f"🎯 *CIERRE POR LÍMITE FIJO*\nPNL: `{pnl_valor:.2f} USDT`\nPrecio: `{precio_actual}`")
            st.rerun()
