...
# 1. INDICADORES Y TÍTULO DINÁMICO
...
tendencia_emoji = "🟢" if precio_actual > st.session_state.precio_anterior else "🔴"
st.session_state.precio_anterior = precio_actual
st.set_page_config(page_title=f"{tendencia_emoji} ${precio_actual:,.0f} | Bot", layout="wide")
...
with st.sidebar:
    ...
    # Diagnóstico detallado con Distancia EMA
    ...
    if 35 < rsi < 55:
        ...
        if precio_actual > ema:
            ...
            msg_diag = f"🔴 **Filtro EMA:** RSI en 35, pero precio `${precio_actual:,.2f}` está a `${dist_ema:.2f}` bajo la EMA."
            st.warning(msg_diag)
    elif rsi >= 55:
        if precio_actual < ema:
            ...
            msg_diag = f"🔴 **Filtro EMA:** RSI en 55, pero precio `${precio_actual:,.2f}` está a `${dist_ema:.2f}` sobre la EMA."
            st.warning(msg_diag)
...
# --- LÓGICA DE POSICIÓN ACTIVA (INDICADORES RECUPERADOS) ---
...
if posicion:
    ...
    if precio_actual > 0:
        ...
        # 👇 INDICADOR RECUPERADO
        ind = "🟢" if pnl_valor >= 0 else "🔴"
        st.warning(f"**POSICIÓN ACTIVA: {side}** | Entrada: **{entry_p:,.2f}** | PNL: {ind} **{pnl_valor:,.2f} USDT** ({pnl_pct:.2f}%)")
...
if c3.button("⛔ CERRAR POSICIÓN"):
    if posicion:
        client.place_order("BTCUSDT", "SELL" if side == "LONG" else "BUY", str(tamano))
        client.registrar_trade(side, entry_p, precio_actual, 0)
        st.rerun()
