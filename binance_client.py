...
class BinanceClient:
    def __init__(self):
        ...
        # 4. Inicialización del Cliente Binance
        try:
            self.client = Client(
                self.api_key, 
                self.api_secret, 
            ...
            mode_str = "DEMO" if self.is_testnet else "REAL"
            print(f"✅ Cliente Binance vinculado con éxito en modo: {mode_str}")
        except Exception as e:
            print(f"❌ Fallo crítico de conexión Binance: {e}")
            self.client = None
    ...
    def get_account_status(self):
        """Obtiene Balance y PNL de Futuros"""
    ...
    def get_open_positions(self, symbol="BTCUSDT"):
        """Busca posiciones activas en el mercado"""
        ...
        try:
            ...
            print(f"⚠️ Error en posiciones: {e}")
            return None
    ...
    def place_order(self, symbol, side, amount):
        ...
        if not self.client: return None
        try:
            # Forzamos la cantidad a string para evitar errores de precisión
    ...
    def registrar_trade(self, side, entry_p, exit_p, pnl):
        ...
        try:
            ...
            print("💾 Registro en DB completado.")
            return True
        except Exception as e:
            print(f"❌ Error DB al registrar trade: {e}")
            return False

    def obtener_historial_db(self):
        """Carga el historial usando el motor SQLAlchemy"""
        try:
    ...
    def enviar_telegram(self, mensaje):
        """Envía notificaciones instantáneas"""
        try:
