...
class BinanceClient:
    ...
    def registrar_trade(self, side, entry_p, exit_p, pnl):
        """Guarda el trade con los nombres de columna correctos."""
        try:
    ...
    def obtener_historial_db(self):
        ...
        try:
            conn = psycopg2.connect(self.db_url)
            # Query sincronizada con la estructura de la tabla
    ...
    def _init_db(self):
        """Inicializa la tabla con todas las columnas necesarias."""
        try:
            conn = psycopg2.connect(self.db_url)
            cur = conn.cursor()
            cur.execute("""
